...
- **Englische Version** — **gut im Schwung (Sessions 28–36 + 44, 13 EN-Seiten live).** Komplett übersetzt: `en/index.html`, drei Kategorien (`i-history`, `i-dentity`, `i-grow`), `en/features.html` (+ `js/features-en.js`), `en/ueber-uns.html`, `en/faqs.html` (FAQPage Schema), `en/user-guide.html` (Wizards + Mickey-Hand), **`en/stadtrundgaenge.html` Hub (Leaflet-Karte) + 4 EN-Stadtseiten (dornbirn, feldkirch, hard, au)**. British English (en_GB), Brand-Vokabular final, Sprachschalter + hreflang in DE-Schwester-Seiten, sitemap mit hreflang-Annotationen pro URL — `build.py update_sitemap` rendert ab Session 44 automatisch DE+EN-Stadt-URLs wenn `en/stadtrundgang-{slug}.html` existiert. **Workflow seit Session 30**: Claude übersetzt direkt in derselben Session. **Branch-Strategie seit Session 31**: pro EN-Bereich ein frischer Branch von `origin/main`, Pattern `claude/en-<bereich>`. **Konvention Session 44**: EN-Stadtseiten werden manuell gepflegt — `rundgaenge.js` bleibt einsprachig, bei neuem Rundgang DE-Stadtseite UND `en/stadtrundgang-*.html` händisch nachziehen. Noch offen: **Blog (Index + 5 Artikel)**, **Vitrine (Hub + 22 Artikel)** mit `vitrine.js` `de:/en:`-Feldern + `build.py`-Erweiterung, 404, sowie Tally-Form-Seiten (workflow + kontakt — Maggy muss englische Tally-Form anlegen). Auch offen: `llms.txt` englische Sektion. Status + Workflow + offene Items in `_doku/englische-version.md`.
