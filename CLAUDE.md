...
3. **Redirect-Stubs** (TEIL 5, seit Session 54): Legt pro Top-Level-Seite `X.html` eine Weiterleitung `X/index.html` an (canonical + meta-refresh + JS auf `/X.html`), damit die alten Schrägstrich-URLs `/X/` nicht mehr 404en. Idempotent; überspringt `index`/`404`/`_`-Dateien und überschreibt nie eine echte `index.html`.
...
- **Video-Encode-Rezept: `libx264 -crf 20 -preset slow -an +faststart`** — das einzige von Maggy live freigegebene Setting (CRF 24/30 wurden Session 17 wegen Artefakten an dünnen Linien abgelehnt). Vor jedem Push: Frame-Zoom-Vergleich (3×, dünne Linien) + SSIM ≥ 0,99. **60fps-Handkamera-Footage (ar/emo) wird beim Re-Encode GRÖSSER als die Quelle** (auch mit Denoise) — dort ist Stream-Copy-Schnitt (`-c:v copy` am Keyframe) der Hebel, nicht Kompression. Bilder: JPG q85 progressive für Fotos, vor Push Text-Kanten im Zoom checken. (Lehre Session 51.)
- **Massen-Edits über viele Files NIE mit PowerShell `Get-Content | Set-Content`** — PS 5.1 zerschreibt UTF-8-Sonderzeichen zu Mojibake (`â€"`) und fügt BOM ein. Stattdessen Python-Script mit `encoding='utf-8', newline=''` (+ Worktrees excluden). Kaputte Datei wiederherstellen via Bash `git show HEAD:pfad > pfad` (sauberer Byte-Stream), dann Edit-Tool. (Lehre Session 51.)
...
- **Browser-Headless-Screenshots fuer Hero-Grafiken: `--virtual-time-budget=Nms` ist Pflicht** bei animierten HTML-Inhalten. Brave/Chrome/Edge (Chromium): `brave.exe --headless=new --disable-gpu --no-sandbox --hide-scrollbars --virtual-time-budget=3000 --screenshot=<path>.png --window-size=WxH file://<html-path>`. Ohne `--virtual-time-budget` sieht man Initial-Frames mit `opacity:0` / `scaleX(0)`. Pattern in Session 39 (Artikel-02-Hero aus `grafik_02a_medienkompetenz.html`). **Windows-Caveat (Session 49):** Brave-Headless crasht hier reproduzierbar (Network-/GPU-Process, exit -1073741819) und `--headless=new` schreibt den Screenshot gar nicht — zuverlaessig ist **Chrome `--headless=old`** (`--disable-gpu --disable-software-rasterizer --disable-dev-shm-usage --force-device-scale-factor=2 --screenshot=… --window-size=WxH`). Bei statischen (nicht-animierten) Renders ist `--virtual-time-budget` dann nicht noetig.
...
- **Vitrine-Grid-Thumbnails sind monochrom S/W** (es gibt KEINEN CSS-`grayscale()`-Filter — die bestehenden Thumbs sind echte S/W-Quelldateien). Neue Kacheln entsprechend: bei einer Marken-Grafik mit Glow/Streak-Deko **im Light-Theme NEU rendern** (weisser BG, schwarze Schrift) statt das Farb-Raster zu invertieren — `ImageOps.invert` macht aus dunklen Deko-Elementen graue Schimmer + sichtbare Linien (Session-49-Vorfall: Maggy bemerkte „komischer Strich" = mein invertiertes `.hc-streak`). Thumb quadratisch (1080×1080, `object-fit:cover`). **Eine Vitrine-Kachel darf direkt auf einen Blog-Artikel zeigen** (`link: "blog/X.html"`) statt eine Duplikat-`vitrine/X.html` anzulegen — vermeidet doppelten Content; `build.py` setzt fuer EN automatisch `../` davor + `hreflang="de"` solange keine EN-Artikelvariante existiert. Lehre Session 49 (OeAD/„Aufwachsen mit KI"-Kachel).
- **App-Rundgang-Links (`iappear.app/.../routes/...`) sind direkt im HTML hardcodiert**, NICHT aus `rundgaenge.js` generiert (die speist nur Counts/Chips via build.py). Link-Aenderungen = reine HTML-Edits, kein build.py noetig; betroffen sind Kategorie-Seiten (`i-history/i-dentity/i-grow.html`) + Stadtseiten (`stadtrundgang-*.html`), je DE + EN. **Vor dem Einsetzen mit `curl -sIL` verifizieren** — die App macht **echte 404s** (kein Catch-all-SPA), tote Slugs fallen also sofort auf. Drei UNABHAENGIGE Namenssysteme pro Rundgang nicht verwechseln: (1) App-Route-Slug im Link z.B. `innenstadt-erleben`, (2) Karten-Stations-Key (`data-rundgang` + `rundgang-stationen.js`) z.B. `innenstadt-erkunden`, (3) sichtbarer H3-Titel „Innenstadt erleben". Site nutzt durchgaengig **non-www** (`iappear.app` → 301 → `www`). Lehre Session 50 (Dornbirn Oberdorf/Innenstadt-Platzhalter + tote Slugs gefixt).
...
- **Interaktive Daten-Viz-Widgets liegen im separaten Repo `iappear-embeds`** (`github.com/wuola/iappear-embeds`, GitHub Pages aktiv) und werden per `<iframe>` auf die **Live-URL** eingebunden (z.B. `https://wuola.github.io/iappear-embeds/dornbirn-wachstum/`) — NICHT die (oft MB-grossen GeoJSON/MapLibre-)Assets ins Website-Repo kopieren. Cross-Origin → feste iframe-Hoehe (ResizeObserver-Autohoehe greift nicht). Embeds mit eigenem Mobile-Breakpoint (`@media max-width:760px`): figure auf Desktop breiter aus der Textspalte ausbrechen lassen (`width:min(1120px,94vw);left:50%;transform:translateX(-50%)`), sonst bleibt das gestapelte Mobile-Layout aktiv (Legende/Steuerung unter der Falte). **Bei „bau die HTML zu X ein" nach dem Datei-Pfad FRAGEN, nicht eine andere HTML substituieren.** Lehre Session 52 (Siedlungs-Animation `dornbirn-wachstum`).
- **Daten-Viz-Making-of-Blogposts: Tabellen/Legenden/Formeln als echtes HTML, nicht als Slide-Screenshot.** Datenquellen-Tabellen + Farb-Legenden mit `<table>` + farbigen `<i>`-Swatches nachbauen, Swatch-Farben per Pixel-Sample aus der Original-Legende ziehen (Magma/Inferno-Colormap). Weiss-hinterlegte Slide-Grafiken (z.B. klassifizierte Gebaeude-Footprints) per Whiteness-Knockout auf den dunklen Seiten-BG freistellen (lum>205 & niedrige Saturation → dark-BG, Rest bleibt Farbe), NICHT `ImageOps.invert` (Halos). Formeln (NDVI etc.) als zentrierter Mono-Block. Seiten-eigener `<style>` inline (kein Cache-Bust ueber alle HTMLs). PPTX-Quelle via `pptx`-Skill (`unpack.py`) auslesen, Medien aus `ppt/media/`. **Video-Hero aus dichtem 3D-Render: 4K→1280, CRF22** als Balance (CRF20 ~8MB zu schwer, CRF25 ~4MB weicht an duennen Kanten/Text auf — 3x-Zoom-Vergleich gemacht); Poster-Frame = og:image. Erstmals Session 53 (`dornbirn-stadtwachstum-3d-geodaten`). **Preview-Screenshot haengt an Cross-Origin-iframes (Live-Embeds nie "network idle")** → stattdessen Chrome-`--headless=old`-Render einer iframe-Platzhalter-Kopie ODER `preview_eval`-DOM-Checks; `fetch` im Preview cached, also Cache-Bust-Query + `cache:'no-store'`.
- **Einen neuen Blog-Artikel veroeffentlichen = kompletter Standard-Prozess in EINEM Rutsch, ohne Rueckfrage, ohne Dev-Jargon** (Maggy-Lehre Session 52 — „verdrahten" + Nachfragen statt Fertigmachen war falsch). „Wie immer auf die Seite stellen" heisst alle 5 Schritte: (1) Artikel-HTML in `/blog/`, (2) Card in `blog/index.html` (`data-pin-label="Neu"`, vorherige „Neu" entpinnen) + JSON-LD `blogPost`-Eintrag, (3) Blog-Teaser in `vitrine.html` (max 4 Cards, aeltester raus, Pin synchron zu /blog/), (4) `sitemap.xml` (prio 0.7; DE-only: hreflang de+x-default), (5) `llms.txt` DE-Blog-Sektion. Diese Edits liegen ausserhalb der build.py-Marker → build.py-safe.
...
**🚀 Blog LIVE seit 2026-05-11 ~08:50 (Session 39), 8 Hauptartikel seit Session 53.** 8 Hauptartikel + 1 Folge-Beitrag unter `/blog/` (Definition / Medien-und-Demokratie-Pflichtfach / i.grow-Workflow / Frauenspuren + Frauenspuren-Live-Launch-Folge / **AR-iOS-vs-Android-Technik-Artikel** (Kategorie "Technik", Session 48 — erster Video-Hero + erste echte Code-Bloecke) / **125 Jahre, 125 Orte** (i.history, Session 52 — Jubilaeums-Rundgang-Release Dornbirn, 3 Event-Fotos + eingebettete Siedlungs-Animation per iframe aus dem `iappear-embeds`-Repo) / **Von der Urmappe zur 3D-Datenvisualisierung** (Kategorie "Technik", Session 53 — Making-of der Siedlungs-Animation aus oeffentlichen Geodaten: NDVI / Random-Forest / Laserscan DOM-DGM / QGIS / Blender; zweiter Blog-Video-Hero, Daten-Tabelle + NDVI-Formel als echtes HTML, Bilder + 3D-Render-Loop aus PPTX; Forward-Link vom 125-Jahre-Artikel) — alle neueren DE-only). Daten 15.3.–8.5.2026 (+ 3.6. fuer AR-Artikel) fuer "gewachsenen" Look. Eigener Blog-Index + Reaktivierung in `vitrine.html`. 4 interaktive Grafiken als iframes mit JS-Auto-Resize (Artikel 02 hat nur noch grafik_02b im Text, weil 02a als Hero-Screenshot rausgezogen wurde). Sitemap + llms.txt erweitert. Tag-System ueberarbeitet (keine Grossbuchstaben, Header-Tags klickbar, Tag-Wolken farbig + bis 22 Tags). Hero-Bild-Workflow per PIL etabliert. Folge-Beitrag-System mit eigenstaendigen URLs, hardcoded Pflege in 3 Stellen (Hauptartikel + Folge-Artikel + Uebersichts-Disclosures), CSS-Klassen `.article__series` / `.article__parent-link` / `.blog-card-series`. Hero-Bilder pro Artikel: 01 PIL-Phone-Mockup (2 Phones), 02 Screenshot der ICILS-Grafik, 03 Foto i.grow Feldkirch, 04 PIL-Phone-Mockup v4 (Aspect-Bug-Fix), Folge-Beitrag Eroeffnungsfoto. Status + Lehren in Sessions 37, 38, 39 (`_doku/sessions/`).

**Google Search Console (Session 26, 2026-05-04 ~17:00):** Property `iappear.at` eingerichtet, `sitemap.xml` eingereicht (41 Seiten verarbeitet, ohne Fehler), 7 Top-URLs manuell zur Indexierung beantragt (Startseite + Stadtrundgaenge + 3 Kategorien + Features + Vitrine-Hub + Klassenraeumen-Artikel) — alle grün. Davor: Pretty-URL-Bug gefixt (Commit `5ebc850`, alle canonical/og:url/sitemap/llms.txt/build.py URLs auf `.html`). **Stand 2026-05-08 (Session 36.5):** 8 indexiert / 43 nicht — davon 39 sind Pretty-URL-Geister aus Google-Discovery-Queue VOR dem Session-26-Fix (`/agb/`, `/datenschutz/`, `/features/`, `/kontakt/`, `/stadtrundgaenge/`, `/stadtrundgang-au/` etc.). Code-Audit verifiziert: 0 Pretty-URL-Quellen mehr im Live-Code (Sitemap, llms.txt, canonical, og:url, internal links, build.py alle sauber). **Keine Aktion nötig** — schmilzt in 2-8 Wochen weg, parallel wachsen die `.html`-URLs in den Index. Bei künftigen "X nicht indexiert"-Schrecksekunden: erstmal Stichprobe der URLs anschauen, dann Code-Audit-Greppattern aus Session 36.5 wiederholen, statt zu raten. **Update Session 54 (2026-06-23):** Die 404-Geister waren bis Anfang Juni noch in GSC sichtbar (15 betroffen = 11 Hauptseiten + 4 Stadtseiten, Trend wachsend weil Google den Discovery-Rückstau abarbeitete). Statt weiter zu warten → aktiv gefixt mit Redirect-Stubs (siehe build.py TEIL 5 + Pretty-URL-Konvention). 404 → 200-Weiterleitung, live verifiziert. Merke für GSC-Lesen: nur „Nicht gefunden (404)"/„Soft 404"/„Serverfehler" sind echte Probleme; „Alternative Seite mit kanonischem Tag" (= `/index.html` vs Verzeichnis-Root dedupliziert) und „Seite mit Weiterleitung" sind das System, das korrekt arbeitet — kein Handlungsbedarf.
